    ...
    <div class="items">
        ...
        <ul class="color-checker">
          <li v-for="(color, i) in defaultItem.settings.colors" :key="i">
            <a
              ...
              v-bind:class="{ active: modelToChange.color === color.hex }"
              :title="color.name"
              v-on:click="changecolor(color.hex)"
            ></a>
          </li>
        </ul>
      </div>
      <div class="elements">
        <div class="grid-row variants">
          <div
            class="item"
            v-bind:class="{ active: activeElement === null }"
            v-on:click="
              setActive(null, { name: 'Основной', model_name: 'all' })
            "
          >
            <img
              :src="itemImage"
          ...
          <div
            class="item"
            v-for="(element, i) in defaultItem.settings.elements"
            :key="i"
            v-bind:class="{ active: i === activeElement }"
            v-on:click="setActive(i, element)"
          >
            <img
              :src="itemImage"
              width="124"
...
export default {
  ...
  methods: {
    setActive: function (index, element) {
      ...
      let el = this.editedItem.settings.find(
        (setting) => setting.model_name === element.model_name
      );
    ...
    changecolor: function (color) {
      ...
      if (this.modelToChange.model_name === "all") {
        for (let i = 0; i < this.defaultItem.settings.elements.length; i++) {
          ...
          this.$eventHub.$emit(
            "dummy:changecolor_tshirt",
            this.defaultItem.settings.elements[i].model_name,
            color
          );
        }
      } else {
    ...
    clearColors: function () {
      ...
      this.defaultItem.settings.elements.forEach((element) => {
        ...
        const defaultColor = this.editedItem.defaults.find(
          (item) => item.model_name === element.model_name
        ).color;
        this.$eventHub.$emit(
          "dummy:changecolor_tshirt",
          itemName,
